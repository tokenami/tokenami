...
function init(modules: { typescript: typeof tslib }) {
	...
	function getSelectorCompletions(
		...
		return TokenamiConfig.properties.flatMap((property) => {
			...
			return [
				...entries,
				...selectorsEntries.map(createVariantPropertyEntry(property)),
			];
		});
	}
	...
	function create(info: tslib.server.PluginCreateInfo) {
		...
		ts.sys.watchFile?.(
			configPath,
			(_, eventKind: tslib.FileWatcherEventKind) => {
			...
			if (sourceFile) {
				ts.forEachChild(sourceFile, function visit(node) {
					if (ts.isPropertyAssignment(node)) {
						const property = ts.isStringLiteral(node.name)
							? node.name.text
							: null;
						const textValue = ts.isStringLiteral(node.initializer)
							? node.initializer.text
							: null;
						const parsedProperty =
							TokenamiConfig.TokenProperty.safeParse(property);

						if (parsedProperty.success) {
							const parts = Tokenami.getTokenPropertyParts(
								parsedProperty.output,
								config,
							);
							...
							const invalidValueIndex = original.findIndex((diagnostic) => {
								const isCodeMatch =
									diagnostic.code === INVALID_VALUE_ERROR_CODE;
							...
							if (invalidValueIndex > -1) {
								...
								if (textValue) {
									const arbitraryValue =
										TokenamiConfig.arbitraryValue(textValue);
									...
									changes: [
										{
											fileName,
											textChanges: [
												{ span: valueSpan, newText: arbitraryText },
											],
										},
									],
								},
