...
interface TokenamiFinalConfig
	extends Merge<TokenamiBaseConfig, TokenamiConfig> {}
...
type CSSPropertyValue<P> = P extends keyof CSS.PropertiesHyphen
	? CSS.PropertiesHyphen[P]
	: never;
...
type TokenName<ThemeKey> = ThemeKey extends keyof ThemeConfig
	? keyof ThemeConfig[ThemeKey]
	: never;
...
type SelectorVariantStyle<P extends string, V> = {
	[key in Tokenami.TokenProperty<
		| `${ResponsiveKey}_${P}`
		| `${SelectorsKey}_${P}`
		| `${ResponsiveKey}_${SelectorsKey}_${P}`
	>]?: V;
};
...
type TokenamiStyle<P extends string, V = PropertyValue<P>> = VariantStyle<
	P,
	V
> & {
...
interface TokenamiProperties
	extends TokenamiBaseStyles,
		UnionToIntersection<TokenamiAliasStyles> {
