...
const run = () => {
	...
	cli
		...
		.action(async (_, flags) => {
			...
			fs.writeFileSync(utils.getTypeDefsPath(configPath), typeDefs, {
				flag: "w",
			});
			fs.writeFileSync(utils.getCiTypeDefsPath(configPath), ciTypeDefs, {
				flag: "w",
			});
		});
	...
	cli
		...
		.action(async (_, flags) => {
			...
			const targets = browserslistToTargets(
				getBrowsersList(projectPkgJson.browserslist),
			);
			let config: Writeable<Tokenami.Config> =
				utils.getConfigAtPath(configPath);

			config.include = flags.files || config.include;
			if (!config.include.length)
				log.error("Provide a glob pattern to include files");

			async function regenerateStylesheet(
				file: string,
...
async function findUsedCssVariables(cwd: string, config: Tokenami.Config) {
	...
	const allCssVariables = entries.flatMap((entry) => {
		...
		const responsiveVariants = findResponsiveCSSUtilityVariables(
			fileContent,
			config.responsive,
		);
		return Array.from(matches, (match) => match.groups?.cssVar).concat(
			responsiveVariants,
		);
	});
...
function findResponsiveCSSUtilityVariables(
	...
	const responsiveCssBlockRegex =
		/css\(([\s\S]*?)\{([\s\S]*?)responsive:\strue([\s\S]*?)\}/g;
	...
	return responsiveCssBlocks.flatMap((block) => {
		...
		const reponsiveVariants = matchesWithoutQuoteMark.flatMap(
			(tokenProperty) => {
				return utils.getResponsivePropertyVariants(
					tokenProperty as any,
					responsiveConfig,
				);
			},
		);
