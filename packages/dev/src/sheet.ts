...
function generate(
	...
	usedTokenProperties.forEach((usedTokenProperty) => {
		...
		const responsive =
			parts.responsive && config?.responsive?.[parts.responsive];
		...
		for (let property of longhands) {
			...
			const styles = {
				[`/*${property}*/${createSelector({ name: parts.name })}`]: hasVariants
					? getVariantStyles(
							isGridProperty ? getGridValue(variantValueVar) : variantValueVar,
					  )
					: getStyles(isGridProperty ? getGridValue(valueVar) : valueVar),
			};

			const gridStyles = isGridProperty && {
				[`/*${property}*/${createGridSelector({ name: parts.name })}`]:
					hasVariants ? getVariantStyles(variantValueVar) : getStyles(valueVar),
			};

			layers.reset["*"] = {
				...layers.reset["*"],
				[Tokenami.tokenProperty(property)]: getResetTokenValue(
					property,
					config,
				),
			};
...
function getResetTokenValue(
	cssProperty: Tokenami.CSSProperty,
	config: Tokenami.Config,
) {
	const aliased = Object.entries(config.aliases || {}).flatMap(
		([alias, properties]) => {
...
function isSupportedProperty(
	property: string,
): property is Tokenami.CSSProperty {
