...
function tokenValue<TK extends string, N extends string>(
	themeKey: TK,
	name: N,
): TokenValue<TK, N> {
...
const GridValue = {
	safeParse: (input: unknown) => v.safeParse(gridValueSchema, input),
};

type TokenProperty<P extends string = string> = `--${P}`;
const tokenPropertyRegexSchema = v.regex(tokenPropertyRegex);
const tokenPropertySchema = v.string([
	tokenPropertyRegexSchema,
]) as v.StringSchema<TokenProperty>;
const TokenProperty = {
	safeParse: (input: unknown) => v.safeParse(tokenPropertySchema, input),
};
...
const tokenValueSchema = v.string([
	tokenValueRegexSchema,
]) as v.StringSchema<TokenValue>;
const TokenValue = {
	safeParse: (input: unknown) => v.safeParse(tokenValueSchema, input),
};
...
type Aliases = Record<
	string,
	readonly (Supports.CSSProperty | (string & {}))[]
>;
...
type Config = DeepReadonly<{
	...
	keyframes?: {
		[name: string]: { [step: string]: { [cssProperty: string]: string } };
	};
	aliases?: Aliases;
	theme: Theme;
...
const createConfig = <T extends Config>(
	obj: Exact<Config, T>,
): DeepReadonly<T> => {
...
function getTokenValueParts(tokenValue: TokenValue) {
	const [, key, token] = tokenValue.split(tokenValueRegex) as [
		string,
		string,
		string,
	];
