...
Tailwind CSS takes another approach to help tick these boxes:
...
- [CSS utility](#user-content-css-utility)
  ...
  - [Configure utility](#user-content-configure-utility)
...
Update `.tokenami/tokenami.config.js` with a glob matching your template files. The watch script will search these files for Tokenami properties to generate necessary styles.
...
Tokenami relies on your theme to provide design system constraints. There aren't any predefined theme values out of the box, so add your own to get the full Tokenami experience. For example:
...
The keys in your `responsive` and `theme` objects can be anything you wish. These keys will be used to name your custom properties (more on this later).
...
1. A Tokenami **value** is any theme key followed by a value identifier. For example, a `color` object in theme with a `red-100` entry maps to `var(--color-red-100)`.
...
module.exports = createConfig({
  ...
  responsive: {
    ...
    'medium-card': '@container (min-width: 400px)',
  },
});
```

Use by following the Tokenami property spec:
...
Use by following the Tokenami value spec:
...
Create a file in your project to configure the utility. You can name this file as you like, e.g. `css.ts`:
...
Now you can import the utility and use it in your components:
...
function Button(props) {
  return <button style={button({ size: 'large' }, props.style)} />;
}
```

The first parameter passed to the `css` utility represents your base styles, the second is for optional variants, and the third parameter enables reponsive variants.

Responsive variants allow you to prefix the variant name with a responsive key from your configuration. For example, the following button will be large at the medium breakpoint:
...
Responsive variants can significantly increase the size of your stylesheet so this is purposefully opt-in to allow greater control.
...
Define custom selectors in the `selectors` object in your config. Use the ampersand (`&`) to specify where the current element's selector should be injected:
...
When using custom aliases, it's recommended to use the CSS utility to ensure that `--p` correctly overrides the properties declared in its array when passed as an override.

### Mapping properties to theme

Tokenami provides sensible defaults to map Tokenami properties to theme keys. For instance, `--border-color` allows values from your `color` object in theme, `--padding` allows multiples of your grid, and `--height` allows values from a `size` key or multiples of your grid.

You can customise this mapping by overriding the `properties` object in your configuration:
...
Use by following the Tokenami property and value spec:
